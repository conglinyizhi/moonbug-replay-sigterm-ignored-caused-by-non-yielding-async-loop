...
**状态：issue 已关闭（[moonbitlang/async#612](https://github.com/moonbitlang/async/issues/612)，2026-09-18），维护者判定为预期行为。**
不是运行时缺陷，而是「在 async 程序里跑不挂起的同步忙循环」的固有后果：运行时受单线程模型所限，对此基本无能为力。

## 维护者回复（中文翻译）

> 这是预期行为。
>
> `moonbitlang/async` 是单线程模型。所以在 `async` 程序里做不带 `@async.pause()` 的同步忙循环，一直都不是个好主意；受单线程执行的限制，运行时对此基本无能为力。
>
> 具体到信号处理：默认情况下 `moonbitlang/async` 会接管 `SIGINT`、`SIGTERM` 等信号，把它们转成运行时自身的异步取消机制，好让用户代码在收到终止信号时能做一些清理，比如删临时文件、终止子进程。但异步取消是协作式的，还在跑的程序只有在挂起时才能开始取消流程。于是程序卡在不挂起的同步循环里时，取消也一样卡住，从外面看起来就跟信号被静默吞掉一样。
>
> 关于你提的两条建议：
>
> - **在全局取消过程中再次收到信号、或过了某个超时之后加入强制杀进程的逻辑**：这本身就是个合理且有用的功能。支持自定义的硬取消超时大概是最好的做法，因为不是所有父进程都会在子进程卡住时重复发终止信号；会重复发的那些，多半本来也备着 `SIGKILL` 的硬兜底。
> - **补文档说明信号处理的注意事项**：其实真正的注意事项是「不要在 async 程序里做不挂起的重同步工作」，这一条文档里是写了的，只是大概不够显眼。信号处理只是「async 程序里跑同步循环」的后果之一，还有一堆别的。
>
> 如果确实必须在 `async` 程序里做重的同步工作（比如用 `@fs` API 读完配置文件，再跑一段纯同步的任务），一个可行的绕过方式是在所有异步工作做完之后，用 `@signal.set_global_cancellation_signal([])` 把所有取消信号关掉。
>
> 个人更偏向硬超时那套做法。对「异步读配置、然后同步干活」这种简单程序，一个合理的硬超时足够让程序正常退出。

评论原文（英文）：[moonbitlang/async#612 (comment)](https://github.com/moonbitlang/async/issues/612#issuecomment-5726046804)

## 以下原文

以下是 issue 关闭时的 README 原文，未作改动。
